...
name: repo-scan
id: repo-scan
description: Use when taking over a large legacy codebase, planning major refactors, auditing third-party dependencies embedded directly in source (vendored, not in package.json), or preparing architecture decision records for monorepo reorganization. Classifies every file (project / third-party / build artifact), detects 50+ embedded libraries, scores modules with four-level verdicts (Core Asset / Extract & Merge / Rebuild / Deprecate), and produces interactive HTML reports. Activates on legacy codebase, structural overview, third-party audit, file classification, monorepo audit, embedded library detection, vendored dependencies, dead code inventory. Skip for small projects, single-file scripts, or new projects from scratch.
category: research
version: 1.1.0
triggers:
  positive:
...
> Every ecosystem has its own dependency manager, but no tool looks across C++,
> Android, iOS, and Web to tell you: how much code is actually yours, what's
> third-party, and what's dead weight.
...
- Optional scope filters: modules, languages, or directories to exclude.
...
2. **Detect embedded libraries**: inspect directory names, headers, license files, version markers to identify bundled dependencies and likely versions.
...
4. **Highlight structural risks**: dead-weight artifacts, duplicated wrappers, outdated vendored code, modules to extract / rebuild / deprecate.
5. **Produce the report**: concise summary plus interactive HTML output with per-module drill-down.
