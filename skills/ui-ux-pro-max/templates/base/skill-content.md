...
- Use **Phosphor (`@phosphor-icons/react`)** as the default icon library. Icons listed in `src/ui-ux-pro-max/data/icons.csv` are common recommendations, not the complete set.
- When the recommendation table does not include a suitable icon:
  - **Prefer choosing a semantically better icon from the full Phosphor set**.
  - If Phosphor does not have a good option, use **Heroicons (`@heroicons/react`)** as a fallback while keeping style consistent (outline/filled, stroke width, corner style).

| Rule | Standard | Avoid | Why It Matters |
|------|----------|--------|----------------|
| **No Emoji as Structural Icons** | Use vector-based icons (e.g., Phosphor `@phosphor-icons/react`, Heroicons `@heroicons/react`, react-native-vector-icons, @expo/vector-icons). | Using emojis (palette rocket gear) for navigation, settings, or system controls. | Emojis are font-dependent, inconsistent across platforms, and cannot be controlled via design tokens. |
